...
function ThemeToggleButton({ theme, mounted }: { theme: string | undefined; mounted: boolean }) {
  const { setTheme } = useTheme();

  if (!mounted) return <div className="w-[1.2rem] h-[1.2rem]" />;

  return (
    <div className="hidden sm:flex items-center">
      <button
        type="button"
        className="px-2 cursor-pointer z-50 relative"
        aria-label="Toggle theme"
        title={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}
        onClick={() => setTheme(theme === "dark" ? "light" : "dark")}
      >
        {theme === "dark" ? (
          <SunIcon className="size-[1.2rem] text-slate-200 cursor-pointer" />
        ) : (
          <MoonIcon className="size-[1.2rem] text-[#08090a] cursor-pointer" />
        )}
      </button>
    </div>
  );
  ...
  return (
    <>
      ...
      >
        ...
        {/* Theme Toggle */}
        <ThemeToggleButton theme={theme} mounted={mounted} />
